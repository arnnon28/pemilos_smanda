    ...
    reader.onload = function (e) {
        ...
        img.onload = function () {
            ...
            const performCompression = () => {
                ...
                while (sizeInKb > targetKb && quality > 0.15) {
                    quality -= 0.05;
                    compressedBase64 = canvas.toDataURL('image/jpeg', quality);
                    sizeInKb = (compressedBase64.length * 3 / 4) / 1024;
                }
...
function processLogoFile(event) {
    ...
    compressImageToLimit(file, maxDim, 50, function (base64Data, shortId, sizeInKb) {
...
async function downloadVoterCardsPdf() {
    ...
    for (let i = 0; i < filteredRecords.length; i++) {
        ...
        try {
            const qr = new QRious({
                value: 'https://arnnon28.github.io/family-money/',
                size: 200,
                level: 'M'
            });
        ...
        doc.setFontSize(7.5);
        doc.text('Website: https://pilkasis.gamer.gd', posX + kartuLebar / 2, posY + kartuTinggi - 5.5, { align: 'center' });
        doc.setFillColor(34, 102, 170);
...
async function updateDashboard() {
    ...
    try {
        ...
        const pcn = stats.total > 0 ? ((stats.masuk / stats.total) * 100).toFixed(2) : 0;
        ...
        if (tPartBody) {
            ...
            for (const vType of voterTypes) {
                ...
                const pct = s.total > 0 ? ((s.masuk / s.total) * 100).toFixed(2) : 0;
                htmlPart += `
            ...
            htmlPart += `
                        <tr class="bg-slate-50 border-t-2 border-slate-200">
                            <td class="py-3 px-3 border-x border-gray-200 text-center font-bold text-slate-800">TOTAL</td>
                            <td class="py-3 px-3 border-x border-gray-200 text-center font-bold text-slate-800">${stats.total}</td>
                            <td class="py-3 px-3 border-x border-gray-200 text-center font-bold text-emerald-600">${stats.masuk}</td>
                            <td class="py-3 px-3 border-x border-gray-200 text-center font-bold text-rose-500">${stats.belum}</td>
                            <td class="py-3 px-3 border-x border-gray-200 text-center font-bold text-sky-600">${pcn}%</td>
                        </tr>
                    `;
        ...
        if (tRinciBody) {
            if (candidates.length === 0) {
                tRinciBody.innerHTML = '<tr><td colspan="8" class="py-10 text-gray-500 text-center">Belum ada data kandidat...</td></tr>';
            } else {
                candidates.sort((a, b) => {
                    const posA = a.posisi || "";
                    const posB = b.posisi || "";
                ...
                let htmlRinci = '';
                candidates.forEach(c => {
                    const totalPos = votesPerPosisi[c.posisi] || 0;
                    const pct = totalPos > 0 ? ((c.total_suara / totalPos) * 100).toFixed(2) : 0;
                    htmlRinci += `
                                <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
