...
async function startVotingWizard() {
    ...
    try {
        ...
        const draftStr = localStorage.getItem('pemilos_draft_pilihan');
        if (draftStr) {
            try { voterDraftSelections = JSON.parse(draftStr); } catch (e) { }
        } else {
...
async function renderWizardStepVoterPage() {
    ...
    if (isSummary) {
        ...
        if (nextBtn) { nextBtn.classList.remove('hidden'); nextBtn.innerHTML = 'Lanjut <i class="fas fa-arrow-right text-xs"></i>'; }
        const pos = voterConfigPositions[voterCurrentStep];
        const hasSelected = !!voterDraftSelections[pos.id];
